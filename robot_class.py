...
class robot:
    ...
    def sense(self):
        ...
        for ldmk in range(len(self.landmarks)):                 # iterate through all of the landmarks in a world
            landmark_x = self.landmarks[ldmk][0]
            landmark_y = self.landmarks[ldmk][1]
            
            dx = landmark_x - self.x + (self.rand() * self.measurement_noise)    # compute dx and dy, the distances between the robot and the landmark
            dy = landmark_y - self.y + (self.rand() * self.measurement_noise)

            
            if dx <= self.measurement_range:
                if dy <= self.measurement_range:
                    measurements.append((ldmk, dx, dy))    # if dx, dy fall in the range,add them to the measurements list
